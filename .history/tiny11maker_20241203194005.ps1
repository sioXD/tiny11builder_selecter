...
function Bypassing_system_requirements{
...
function Disabling_Sponsored_Apps {
...
function Enabling_Local_Accounts_on_OOBE{
...
function Disabling_Reserved_Storage{
...
function Disabling_BitLocker_Device_Encryption{
...
function Outlook{
    Write-Host "Prevents installation of Outlook:"
...
function DevHome{
    Write-Host "Prevents installation of DevHome:"
...
Write-Host "2 - tiny11 mit edge"
Write-Host "3 - Manual configuration"
Write-Host "4 - tiny11 mit edge und winkey (was not testet)"
$choice = Read-Host "Enter your choice (1, 2, 3, 4)"

switch ($choice) {
    "1" {
        Write-Host "Option 1 selected: tiny11."
        #calling all functions for deleting
        $var = "Edge", "OneDrive", "Load_reg", "Bypassing_system_requirements", "Disabling_Sponsored_Apps", "Enabling_Local_Accounts_on_OOBE", "Disabling_Reserved_Storage", "Disabling_BitLocker_Device_Encryption", "Disabling_Chat_icon", "Edge_registries", "OneDrive_Backup", "Disabling_Telemetry", "Outlook", "DevHome"
    }
    "2" {
        Write-Host "Option 2 selected: tiny11 with edge."
    ...
    "3" {
        Write-Host "Option 3 selected: Manual configuration."
        ...
        $form.Text = "Components selection"
        ...
        $listBox.BackColor = [System.Drawing.Color]::FromArgb(255, 255, 255)  
        $listBox.ForeColor = [System.Drawing.Color]::FromArgb(0, 0, 0)  
        ...
        $components = @(    #! names must not change (used in variables)
            ...
            'microsoft.windowscommunicationsapps_', 'Microsoft.WindowsFeedbackHub_', 'Microsoft.WindowsMaps_', 
            'Microsoft.WindowsSoundRecorder_', 'Microsoft.Xbox.TCUI_', 'Microsoft.XboxGamingOverlay_',
            'Microsoft.XboxGameOverlay_', 'Microsoft.XboxSpeechToTextOverlay_', 'Microsoft.YourPhone_', 
            'Microsoft.ZuneMusic_', 'Microsoft.ZuneVideo_', 'MicrosoftCorporationII.MicrosoftFamily_', 
       ...
       # button with hover effect
        ...
        $okButton.Add_Click({
            ...
            foreach ($index in $listBox.CheckedIndices) {
                $selectedItems += $listBox.Items[$index]
            }
            $global:selectedComponents = $selectedItems     #it's used: $packagePrefixes
            $form.Close()
        })
    ...
    if ($global:selectedComponents) {
        ...
        foreach ($component in $global:selectedComponents) {
            switch ($component) {
                'Edge' { Write-Host "Edge added to var"; $var += "Edge"; $var += "Edge_registries" }
                'OneDrive' { Write-Host "OneDrive added to var"; $var += "OneDrive"; $var += "OneDrive_Backup" }
                'Load_reg' { Write-Host "Reg get leaded"; $var += "Load_reg" } #! should always be selected
                'Bypassing_system_requirements' { Write-Host "Bypassing System Requirements"; $var += "Bypassing_system_requirements" }
                'Disabling_Sponsored_Apps' { Write-Host "Disabling Sponsored Apps"; $var += "Disabling_Sponsored_Apps" }
                'Enabling_Local_Accounts_on_OOBE' { Write-Host "Enabling Local Accounts on OOBE"; $var += "Enabling_Local_Accounts_on_OOBE" }
                'Disabling_Reserved_Storage' { Write-Host "Disabling Reserved Storage"; $var += "Disabling_Reserved_Storage" }
                'Disabling_BitLocker_Device_Encryption' { Write-Host "Disabling BitLocker Device Encryption"; $var += "Disabling_BitLocker_Device_Encryption" }
                'Disabling_Chat_icon' { Write-Host "Disabling Chat Icon"; $var += "Disabling_Chat_icon" }
                'Edge_registries' { Write-Host "removing egde registries"; $var += "Edge_registries" }
                'OneDrive_Backup' { Write-Host "removing OneDrive_Backup"; $var += "OneDrive_Backup" }
                'Disabling_Telemetry' { Write-Host "Disabling Telemetry"; $var += "Disabling_Telemetry" }
...
$packagePrefixes += $global:selectedFeatures
